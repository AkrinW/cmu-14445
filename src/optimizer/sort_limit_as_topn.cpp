...
auto Optimizer::OptimizeSortLimitAsTopN(const AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef {
  ...
  std::vector<AbstractPlanNodeRef> optimized_child;
  for (const auto &child : plan->GetChildren()) {
    optimized_child.emplace_back(OptimizeSortLimitAsTopN(child));
  }
  auto optimized_plan = plan->CloneWithChildren(std::move(optimized_child));
